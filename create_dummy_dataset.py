...
def rhs(tau, X, params):
    ...
    dM = (
        ...
        + 2 * alpha_AE * eta_clf * (CCt @ (M @ D + g * np.outer(s, h)))
        - 2 * alpha_AE * eta_clf * g * np.outer(C, h)
        - 2 * alpha_AE * lW * M
    )
    ds = (
        -2 * alpha_AE * ((T @ M - N.T) @ (Lambda @ h) + kappa * (T @ s) - kappa * a + g * u)
        + 2 * alpha_AE * eta_clf * (CCt @ (M @ (Lambda @ h) + kappa * s))
        - 2 * alpha_AE * eta_clf * g * C
        - 2 * alpha_AE * lW * s
    )
    ...
    dQ = (
        -2 * alpha_AE * aux
        - 2 * alpha_AE * aux.T
        + 2 * alpha_AE * eta_clf * (CCt @ S + S @ CCt)
        - 2 * alpha_AE * eta_clf * g * (np.outer(C, s) + np.outer(s, C))
        - 4 * alpha_AE * lW * Q
    )
    ...
    dt_ = (
        ...
        + 2 * alpha_AE * eta_clf * (CCt @ H)
        - 2 * alpha_AE * eta_clf * g * rho * C
    ...
    dB = (
        ...
        + 2 * alpha_AE * eta_clf * (G @ CCt)
        - 2 * alpha_AE * eta_clf * g * np.outer(a, C)
        - 2 * alpha_AE * (lA + lW) * B
    )
    ...
    for epoch in range(1, config.n_epochs + 1):
        ...
        for start in range(0, n_samples, batch_size):
            ...
            for _ in range(classifier_steps):
                ...
                loss_c.backward()
                c_opt.step()
                clf_loss_acc += float(loss_c.item())
                n_clf_updates += 1
            ...
            loss_ae.backward()
            ae_opt.step()
            ae_loss_acc += float(loss_ae.item())
            n_batches += 1
        ...
        obs_epoch, rec_epoch = compute_measured_observables(
            ae, lat_dis, X, y, U, v, lam, params["eta"], params["g"], device
        )
        del obs_epoch
        epoch_ae_losses.append(ae_loss_acc / max(n_batches, 1))
        epoch_clf_losses.append(clf_loss_acc / max(n_clf_updates, 1))
...
def plot_loss_curve(loss_history, params, out_file=OUT_LOSS):
    """Plot the actual epoch-averaged objectives minimized during training."""
    ...
    axes[0].plot(epochs, ae_losses, color="#f1c40f", lw=2.0, label="AE objective minimized")
    axes[0].plot(epochs, clf_losses, color="#3498db", lw=1.8, label="classifier objective minimized")
    axes[0].set_ylabel("train objective")
    axes[0].set_title("Training objectives actually minimized")
    axes[0].legend()
