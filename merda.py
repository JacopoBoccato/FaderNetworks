#!/usr/bin/env python3
# ------------------------------------------------------------
# Debug reconstruction loss and tensor alignment (with attributes)
# ------------------------------------------------------------
import torch
...
# 1. User paths
# ------------------------------------------------------------
run_dir = "/home/ipht/jboccato/FADER_TEST/FaderNetworks/models/lattice_dense_minimal/gor8crk124"              # directory with params.pkl and weights
data_path = "/home/ipht/jboccato/FADER_TEST/FaderNetworks/data/lattice/processed/sequences_27.pth"           # one-hot input sequences
attr_path = "/home/ipht/jboccato/FADER_TEST/FaderNetworks/data/lattice/processed/labels_27.pth"     # attribute matrix
...
# ------------------------------------------------------------
# 3. Load data and attributes
# ------------------------------------------------------------
batch_x = torch.load(data_path, map_location=device).float()
batch_y = torch.load(attr_path, map_location=device).float()

print("✅ Loaded data:", batch_x.shape)
print("✅ Loaded attributes:", batch_y.shape)

# Ensure they’re the same batch size
B = min(batch_x.size(0), batch_y.size(0))
batch_x = batch_x[:B].to(device)
batch_y = batch_y[:B].to(device)

# Optionally limit batch for quick debug
batch_x = batch_x[:4]
batch_y = batch_y[:4]
...
# correlation between input and logits
corr = torch.corrcoef(torch.stack([
    recon.detach().view(-1),
    batch_x.view(-1).float()
]))
print("Correlation recon↔x:\n", corr)

# cross-entropy diagnostics
